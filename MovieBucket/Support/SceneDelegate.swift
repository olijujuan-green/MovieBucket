...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func createTabBarController() -> UITabBarController {
        ...
        tabBar.viewControllers  = [createMovieListNC(), createWatchlistNC()]
        return tabBar
    }
    
    func createMovieListNC() -> UINavigationController {
        let movieListVC         = MovieListVC()
        movieListVC.title       = "Home"
        movieListVC.tabBarItem  = UITabBarItem(title: "Home", image: UIImage.init(systemName: "house"), tag: 0)
        
        return UINavigationController(rootViewController: movieListVC)
    }
    
    func createWatchlistNC() -> UINavigationController {
        let watchlistVC         = FavoritesListVC()
        watchlistVC.title       = "Favorites"
        watchlistVC.tabBarItem  = UITabBarItem(tabBarSystemItem: .favorites, tag: 0)
